...
inventory::collect!(registry::RegistryEntry);
        ...
        if tstate.is_null() {
            unsafe {
                ...
                if let Ok(builtins) = py.import("builtins") {
                    if let Err(e) = registry::inject_into_module(py, &builtins) {
                        e.print(py);
                    }
                }
...
if 'turbox' not in sys.modules:
    ...
    m.add_route = lambda *args, **kwargs: None
    m.run_server = lambda *args, **kwargs: None
