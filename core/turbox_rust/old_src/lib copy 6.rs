...
use pyo3::prelude::*;
use pyo3::ffi;
use pyo3::types::{PyModule, PyString, PyDict, PyBool, PyBytes}; 
...
fn inject_rust_functions(py: Python) -> PyResult<()> {
    let builtins = py.import("builtins")?;
    builtins.setattr("printline", wrap_pyfunction!(printline, py)?)?;
    Ok(())
}
    ...
    WORKER_INTERPRETER_STATE.with(|cell| {
        ...
        if tstate.is_null() {
            unsafe {
                ...
                // -------------------------------------
                //   🔥 INSERT UTILS MODULE HERE 🔥
                // -------------------------------------
                let _ = inject_rust_functions(py);
                // -------------------------------------
...
#[pyfunction]
fn printline(_py: Python, msg: String) -> PyResult<()> {
...
fn turbox(_py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(printline, m)?)?;
    Ok(())
}
